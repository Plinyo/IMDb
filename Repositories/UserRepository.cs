    ...
    public class UserRepository
    {
        ...
        public Usuario Get(string username, string password)
        {
           var userBR = new UserRepository();
            return userBR.GetAll().Where(x => x.NomeUsuario.ToLower() == username.ToLower() && x.Senha.ToLower() == password.ToLower()).FirstOrDefault();
        }
        public Usuario GetByName(string username)
        {
            var userBR = new UserRepository();
            return userBR.GetAll().Where(x => x.NomeUsuario.ToLower() == username.ToLower()).FirstOrDefault();
        }
        public Usuario GetById(int id)
        {
            var userBR = new UserRepository();
            return userBR.GetAll().Where(x => x.Id == id).FirstOrDefault();
        }       
    }
}